...
public class Quiz {
    ...
    public void startQuiz(Student student, Subject subject) {
        ...
        for (int i = 0; i < questions.size(); i++) {
            if (!isOngoing) { // Kiểm tra nếu bài thi đã bị dừng
                System.out.println("project.Quiz stopped due to time up or user interruption.");
                break;
            }
            ...
            while (isOngoing) {
                ...
                if (!isOngoing) { // Dừng lại nếu quiz đã dừng khi nhập xong
                    System.out.println("project.Quiz stopped due to time up.");
                    break;
                }
    ...
    public void printInfo() {
        for (Question question : questions) {
            System.out.println("\n    project.Question: " + question.getContent());
            System.out.println("    Difficulty: " + question.getDifficulty());
            System.out.println("    Options: " + question.getOptions());
            System.out.println("    Correct project.Answer: " + question.getCorrectAnswer());
        }
    }
